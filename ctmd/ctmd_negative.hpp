...
inline constexpr void negative_impl(const in_t &in, out_t &out) noexcept {
    out[] = -in[];
}
...
inline constexpr void negative(const in_t &in, out_t &out,
    ...
    core::batch(detail::negative_impl<decltype(uin), decltype(uout)>,
                std::make_tuple(uin), std::make_tuple(uout),
                std::make_tuple(core::to_mdspan(in)),
