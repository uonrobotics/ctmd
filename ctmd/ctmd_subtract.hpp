...
inline constexpr void subtract_impl(const in_t &in, const out_t &out,
...
inline constexpr void subtract(const in_t &in, const val_t &val, out_t &out,
                               const MPMode mpmode = MPMode::NONE) noexcept {
    ...
    case MPMode::NONE:
        core::batch<decltype(bexts)::rank(), MPMode::NONE>(
            detail::subtract_impl<decltype(ubin), decltype(ubout), val_t>,
...
template <md_c in_t, typename val_t>
    requires(std::is_arithmetic_v<val_t>)
[[nodiscard]] inline constexpr auto
subtract(const in_t &in, const val_t &val,
         const MPMode mpmode = MPMode::NONE) noexcept {
    constexpr auto uin_exts = extents<typename in_t::index_type>{};
