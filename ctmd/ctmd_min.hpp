...
inline constexpr void min_impl(const in_t &in, const out_t &out) noexcept {
    out() = in(0);
...
inline constexpr void min_impl(const in_t &in, out_t &out) noexcept {
    ...
    if constexpr (in_t::rank() == 0) {
        ...
        for (typename in_t::index_type i = 0; i < in.extent(0); i++) {
            min_impl(core::submdspan_from_left(in, i), out);
        }
    }
}
...
min(auto &&In, const ctmd::MPMode mpmode = ctmd::MPMode::NONE) noexcept {
    const auto in = core::to_const_mdspan(std::forward<decltype(In)>(In));
...
[[nodiscard]] inline constexpr auto min(auto &&In) noexcept {
    const auto in = core::to_const_mdspan(std::forward<decltype(In)>(In));
    using in_t = decltype(in);

    typename in_t::value_type out =
        std::numeric_limits<typename in_t::value_type>::max();

    detail::min_impl(in, out);
