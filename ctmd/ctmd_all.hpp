...
inline constexpr void all_impl(const in_t &in, const out_t &out) noexcept {
...
inline constexpr void all_impl(const in_t &in, const out_t &out) noexcept {
    for (typename in_t::index_type i = 0; i < in.extent(in_t::rank() - 1);
         i++) {
        all_impl(core::submdspan_from_right(in, i),
...
all(auto &&In, const ctmd::MPMode mpmode = ctmd::MPMode::NONE) noexcept {
    const auto in = core::to_const_mdspan(std::forward<decltype(In)>(In));
